...
restarts, the proxy reconnects its single upstream **once** — agent front
connections never drop, so the whole fleet no longer reconnect-storms. This is
mcplayer's founding job ("stop agents from spawning their own socat bridges").
